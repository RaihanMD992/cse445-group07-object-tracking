...
import data_engine as tracking_data_engine
...
def load_traffic_csv(csv_path: Path) -> pd.DataFrame:
    """
    Load a processed tracking CSV through data_engine's shared parser.

    This keeps the chatbot, dashboard, explorer, charts, and upload validation
    on one normalization/validation path. CSVs may therefore use supported
    aliases such as frame_id, object_id, person_id, category, etc. instead of
    being limited to the exact headers Frame, Tracking_ID, and Class.
    """
    ...
    dataframe, errors = tracking_data_engine.load_tracking_csv(
        str(csv_path),
        csv_path.name,
    )
    ...
    if dataframe is None:
        message = (
            " ".join(errors)
            if errors
            else f"The file '{csv_path.name}' could not be loaded."
        )
